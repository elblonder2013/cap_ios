...
#import "AplicarCreditoVC.h"
#import "Util.h"
@interface HomeGerenteVC ()
{
    BOOL perfilLoaded;
    Gerente * gerente;
}
...
-(void)PrepareRequestToGetPerfilData
{
    ...
    ASIHTTPRequest* request = [rm prepareRequestToGetWithDictionary:@{@"x-access-token":[GestorV getGV].token, @"id_usuario":[GestorV getGV].uid} ruta:@"gerente"];
...
- (void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    if (codeStatus==200) {
        ...
        if (request.tag==1) {
            //todo correcto
            gerente = [Gerente new];
            [gerente fromDictionary:responseDictionary];
            [GestorV getGV].userAutenticado = gerente;
            [self InitializeData];
            perfilLoaded = YES;
...
-(void)InitializeData
{
    ...
    if (gerente!=nil&&(![gerente.datosUsuario.avatar isEqualToString:@""])) {
        NSString * avatarUrl = gerente.datosUsuario.avatar;
...
/*LA NAVEGACION*/
- (IBAction)GoToPerfilGerente:(id)sender {
    [self HideMenu];
...
- (IBAction)GoToListadoPromotores:(id)sender {
    [self HideMenu];
...
- (IBAction)pickGerentes:(id)sender {
    AplicarCreditoVC * vc = [[AplicarCreditoVC alloc] initWithNibName:@"AplicarCreditoVC" bundle:nil];
    [self.navigationController pushViewController:vc animated:YES];
}
